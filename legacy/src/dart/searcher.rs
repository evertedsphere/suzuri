...
impl SearchStep {
    pub fn to_str(&self, haystack: &str) -> String {
        match *self {
            SearchStep::Match(start, end) => format!("{}", &haystack[start..end]),
            SearchStep::Reject(start, end) => format!("[*{}]", &haystack[start..end]),
        }
    }
}
