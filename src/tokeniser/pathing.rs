...
pub fn shortest_path(
    ...
    debug_assert!((0..node_count)
        .zip(1..node_count)
        .all(|(index, next_index)| get_rank(next_index) >= get_rank(index)));
    ...
    debug_assert!(rank_to_range
        .iter()
        .all(|range| range.start < node_count as u32));
    debug_assert!(rank_to_range
        .iter()
        .all(|range| range.end >= range.start as u32));
    debug_assert!(rank_to_range
        .iter()
        .filter(|range| range.end - range.start > 0)
        .collect::<Vec<_>>()
        .windows(2)
        .all(|window| window[1].start >= window[0].start));
    debug_assert_eq!(
        rank_to_range
            .iter()
    ...
    while (starting_index as usize) < node_count {
        ...
        for index in range.clone() {
            ...
            if current_node_cost == COST_MAX {
                ...
                debug_assert!(range
                    .clone()
                    .all(|index| cost_for_node[index as usize] == COST_MAX));
                break;
            }
